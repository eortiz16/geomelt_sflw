...
void Player::read_buttons(unsigned int button)
{
	...
	/*
	bool(*pressed)(unsigned int, unsigned int) = Joystick::isButtonPressed;

	if (pressed(joyID, buttonMapping.buttons[ATTACK].myID1)
		|| pressed(joyID, buttonMapping.buttons[ATTACK].myID2))
	{
		toggle.attacking ^= 1;
		toggle.attackTimer.restart();
	}

	if (pressed(joyID, buttonMapping.buttons[JUMP].myID1)
		|| pressed(joyID, buttonMapping.buttons[JUMP].myID2))
		jump();
	*/
}
